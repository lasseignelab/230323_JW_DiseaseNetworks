...
setbp1_files <- list.files(path = here(
  "results/SETBP1_Expression/"), pattern = "_targets_tpm.csv", all.files = TRUE, full.names = TRUE)

# read in median TPM counts
tissues_median_tpm <- read.csv(here(
  "results/SETBP1_Expression/aggregated_tissues_tpm_median.csv"), row.names = 1)
...
setbp1_targets <- read.csv(file = here(
  "data/SETBP1_Targets/setbp1_targets_geneconversions.csv"), row.names = 1)
...
tissues_med_log <- tissues_median_tpm %>% 
  ...
  left_join(., setbp1_targets, by = "target") %>% #combine TPM w/ gene annotations
...
clu_df <- lapply(names(clust_list), function(i){
  out <- data.frame(GeneID = targets_tissue[clust_list[[i]],1],

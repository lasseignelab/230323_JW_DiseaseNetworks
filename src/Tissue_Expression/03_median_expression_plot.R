...
sb1_tissue_plot <- ggplot(setbp1_med_tpm_vp,
                          aes(x = TPM, y = reorder(Tissue, TPM, FUN = median),
                              fill = Affected)) +
...
gtex_meta <- read.delim(
  here("data/GTEx_Analysis_v8_Annotations_SampleAttributesDS.txt"))
brain_meta <- filter(gtex_meta, SMTS == "Brain")
brain_tpm <- read.csv(
  here("results/SETBP1_Expression/BRAIN_SETBP1_targets_tpm.csv"), row.names = 1)

# find brain regions
brain_regions <- brain_tpm %>%
  pivot_longer(.,
               cols = starts_with("GTEX"),
               names_to = "SAMPID", values_to = "TPM") %>%

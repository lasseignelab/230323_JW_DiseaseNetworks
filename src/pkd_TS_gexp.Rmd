...
# row_anno <- pkd_gene_anno %>% column_to_rownames(., var = "gene") %>% arrange(desc(Multiplicity)) %>% mutate(., gene = factor("gene", levels = "gene"))  %>% select(., Disease = disease, TF = AlsoTF, Multiplicity = Multiplicity)

row_anno <- pkd_gene_anno %>% arrange(desc(Multiplicity)) %>% mutate(., gene = factor(gene, levels = gene))  %>% select(., Gene = gene, Disease = disease, TF = AlsoTF, Multiplicity = Multiplicity)
...
tis_prop_corr$estimate
tis_prop_corr$p.value
```

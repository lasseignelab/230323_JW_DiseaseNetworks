...
```{r load-CollecTRI-prior}
# filepath to CollecTRI prior
...
save(data, file = here("results/tf_activity/data.Rdata"))
write.csv(data, file = here("results/tf_activity/tf_activity_scores.csv"))
```
...
Variance is mean squared difference between each data point and the center of the distribution measured by the mean. More specifically, variance measures how far each number in the set is from the mean (average), and thus from every other number in the set.
```{r calculate-row-wise-variance}
library(matrixStats)
variance <- rowVars(data)
tf_names <- rownames(data)

var_df <- data.frame(tf_names, variance)

summary(var_df$variance)
```

```{r calculate-col-wise-variance}
col_variance <- colVars(data)
tissues <- colnames(data)

col_var_df <- data.frame(tissues, col_variance)
```
...
```{r style-file}
# run style
style_file(here("src/tf_activity/02_TF_activity_GTEx.Rmd")) # commented out after being run once

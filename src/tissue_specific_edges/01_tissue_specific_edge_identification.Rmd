...
tissue_specific$tiss_name <- paste0(tissue_specific$gene, "_", tissue_specific$Tissue) # add tissue to gene name for mapping
tse_genes <- as.vector(unique(tissue_specific$tiss_name))
...
subset_dfs <- lapply(names(condor_list), function(list_name) {
  ...
  df$tiss_name <- paste0(df$name, "_", list_name)
  subset_df <- df[df$tiss_name %in% tse_genes, ]
...
# reorder and plot
affected_merged$multiplicity_group <- factor(affected_merged$multiplicity_group, levels = c("5", "6", "12", "13", "15+"))
...
ggplot(affected_merged, aes(x = tissue_type, fill = multiplicity_group)) +
  ...
  scale_fill_manual(
  values = c("#D3B1C5", "#C06CB4", "#A468A9", "#533180", "#2C1D6C"),
  labels = c("5", "6", "12", "13", "15+"), # note: there was no 3 in this data
  name = "Multiplicity"
  ) +
...
#   5.00   30.00   31.00   29.02   31.00   31.00 

# filtering to excude edges that have a multiplicity of less than Q1
affected_merged <- affected_merged[affected_merged$multiplicity < 30, ]

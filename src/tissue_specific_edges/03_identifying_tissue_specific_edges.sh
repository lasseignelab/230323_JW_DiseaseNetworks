#!/bin/bash
## run the Rscript 03_identifying_tissue_specific_edges.R and schedule this job to SLURM with
## `sbatch 03_identifying_tissue_specific_edges.sh`
...
singularity exec --cleanenv --no-home -B ${wd} ${wd}/bin/downstream_analysis_docker/setbp1_manuscript_1.0.6.sif Rscript --vanilla ${wd}src/tissue_specific_edges/03_identifying_tissue_specific_edges.R